// 封装画布绘制基础API，目前提供绘制路径、图片、文字、生成图片4个API
 ...
 * size    路径大小，如2
 */
ZZCanvas.prototype.drawPath = function(option){
  const {
    paths = [],
    color = 'black',
    size = 2,
 ...
 * height  图片高度
 */
ZZCanvas.prototype.drawImage = function(option){
  const {
    src,
    x = 0,
    y = 0,
 ...
 * maxWidth  文字最大宽度
 */
ZZCanvas.prototype.drawText = function(option){
  const {
 ...
 * @returns 文本宽度
 */
ZZCanvas.prototype.getTextWidth = function(text, font){
  const {
    fontFamily = 'sans-serif',
    fontWeight =  'normal',
    size = '14px',
 ...
 */
ZZCanvas.prototype.toDataUrl = function(type = 'image/png', level = 0.92){
  return canvasEle.toDataURL(type, level);
}

module.exports = ZZCanvas;
